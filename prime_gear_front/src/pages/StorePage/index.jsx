...
function StorePage() {
      ...
      if (location.state?.categoryIdentifier) {
        ...
        const matchedCategory = categorias.find((cat) => {
          ...
          return (
            ...
            (identifier === "perifericos" && (catName.includes("periférico") || catName.includes("periferico") || catName.includes("periféricos") || catName.includes("perifericos")))
          );
        });
  ...
  async function obterProdutosFiltrados() {
    try {
      ...
      const response = await axios.post("http://primegear.site:8080/produtos-filtrados", filtros);
